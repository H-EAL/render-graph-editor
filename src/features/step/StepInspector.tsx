...
import type { Step, RasterStep, ColorAttachment, DepthAttachment, LoadOp, StoreOp } from '../../types';
...
function ColorAttachmentRow({ idx, att, step }: { idx: number; att: ColorAttachment; step: RasterStep }) {
  ...
  const updateAtt = (patch: Partial<ColorAttachment>) => {
    ...
    updateStep(step.id, { attachments: { ...step.attachments, colorAttachments } } as Partial<Step>);
  };
  ...
  if (!dep) {
    return (
      <div className="px-3 pb-2">
        <button
          onClick={() => updateDep({ target: '', loadOp: 'clear', storeOp: 'store', clearValue: 1 })}
    ...
    <div className="bg-zinc-800/40 border border-zinc-700/40 rounded mx-3 mb-2 overflow-hidden">
      ...
      {dep.loadOp === 'clear' && (
        <FieldRow label="Clear Value">
          <input
            type="number" step="0.01" value={dep.clearValue}
...
function RasterStepEditor({ step }: { step: RasterStep }) {
  ...
  const addColorAttachment = () => {
    const colorAttachments = [
      ...
      { target: '', loadOp: 'clear' as LoadOp, storeOp: 'store' as StoreOp, clearValue: [0, 0, 0, 1] as [number, number, number, number] },
    ];
    updateStep(step.id, { attachments: { ...step.attachments, colorAttachments } } as Partial<Step>);
  };
...
export function StepInspector() {
  const { pipeline, resources, selectedStepId, selectedCommandId, updateStep, selectStep, selectCommand } = useStore();
  ...
  return (
    <div className="flex flex-col overflow-y-auto h-full">
      ...
      <InspectorSection title={step.type === 'raster' ? 'Attachments' : `${step.type} Settings`}>
