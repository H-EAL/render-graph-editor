...
export function validateDocument(pipeline: Pipeline, resources: ResourceLibrary): ValidationIssue[] {
  ...
  for (const tl of pipeline.timelines) {
    if (timelineIds.has(tl.id)) {
      issues.push({ id: newId(), severity: 'error', message: `Duplicate timeline ID: "${tl.id}"`, location: 'Timelines' });
    }
    timelineIds.add(tl.id);

    if (!tl.name.trim()) {
      issues.push({ id: newId(), severity: 'warning', message: `Timeline "${tl.id}" has no name`, location: 'Timelines' });
    }
  }
  ...
  for (const [pid, pass] of Object.entries(pipeline.passes)) {
    if (passIds.has(pid)) {
      issues.push({ id: newId(), severity: 'error', message: `Duplicate pass ID: "${pid}"`, location: pass.name });
    }
    passIds.add(pid);
    ...
    if (pass.steps.length === 0 && pass.reads.length === 0 && pass.writes.length === 0) {
      issues.push({ id: newId(), severity: 'warning', message: `Pass "${pass.name}" has no steps and no resource accesses`, location: pass.name });
    }

    // Read/write references
    for (const rid of pass.reads) {
      if (!allResourceIds.has(rid)) {
        issues.push({ id: newId(), severity: 'warning', message: `Pass "${pass.name}" reads unknown resource "${rid}"`, location: pass.name });
      }
    }
    ...
    for (const sid of pass.steps) {
      if (!pipeline.steps[sid]) {
        issues.push({ id: newId(), severity: 'error', message: `Pass "${pass.name}" references missing step "${sid}"`, location: pass.name });
      }
    }
  }
  ...
  for (const tl of pipeline.timelines) {
    for (const pid of tl.passIds) {
      if (!pipeline.passes[pid]) {
        issues.push({ id: newId(), severity: 'error', message: `Timeline "${tl.name}" references pass "${pid}" which does not exist`, location: tl.name });
      }
    }
  }
  ...
  for (const [, step] of Object.entries(pipeline.steps)) {
    if (stepIds.has(step.id)) {
      issues.push({ id: newId(), severity: 'error', message: `Duplicate step ID: "${step.id}"`, location: 'Steps' });
    }
    stepIds.add(step.id);

    for (const rid of step.reads) {
      if (!allResourceIds.has(rid)) {
        issues.push({ id: newId(), severity: 'warning', message: `Step "${step.name}" reads unknown resource "${rid}"`, location: step.name });
      }
    }
    for (const rid of step.writes) {
      if (!allResourceIds.has(rid)) {
        issues.push({ id: newId(), severity: 'warning', message: `Step "${step.name}" writes unknown resource "${rid}"`, location: step.name });
      }
    }
    ...
    if ((step.type === 'copyImage' || step.type === 'blitImage' || step.type === 'resolveImage')) {
      if (step.source && !allResourceIds.has(step.source)) {
        issues.push({ id: newId(), severity: 'error', message: `Step "${step.name}" references unknown source "${step.source}"`, location: step.name });
      }
      if (step.destination && !allResourceIds.has(step.destination)) {
        issues.push({ id: newId(), severity: 'error', message: `Step "${step.name}" references unknown destination "${step.destination}"`, location: step.name });
      }
    }
    if (step.type === 'generateMipChain' && step.target && !allResourceIds.has(step.target)) {
      issues.push({ id: newId(), severity: 'error', message: `Step "${step.name}" references unknown target "${step.target}"`, location: step.name });
    }
    if (step.type === 'fillBuffer' && step.target && !allResourceIds.has(step.target)) {
      issues.push({ id: newId(), severity: 'error', message: `Step "${step.name}" references unknown buffer "${step.target}"`, location: step.name });
    }
  }
  ...
  for (const rid of resIds) {
    if (seenRes.has(rid)) {
      issues.push({ id: newId(), severity: 'error', message: `Duplicate resource ID: "${rid}"`, location: 'Resources' });
    }
    seenRes.add(rid);
  }
  ...
  try {
    ...
    for (const passId of Object.keys(pipeline.passes)) {
      if (!visited.has(passId) && hasCycle(passId)) {
        issues.push({ id: newId(), severity: 'error', message: `Dependency cycle detected involving pass "${pipeline.passes[passId]?.name ?? passId}"`, location: 'Dependencies' });
        break;
      }
    }
  } catch {
